...
<body>
	...
	<div class="page-body">
		...
		<div class="page-content">
			...
			<%
				List<AnnouncementBean> announcementBeanlist = (List<AnnouncementBean>) request
						.getAttribute("announcementBeanlist");
			%>
			<form action="SystemAnnouncementServlet">
				<table>
					<tr>
						<td>标题：</td>
						<td><input type="text" id="search_announcement_name"
							name="search_announcement_name"
							value="<%=request.getAttribute("search_announcement_name") != null ? request
					.getAttribute("search_announcement_name") : ""%>" /></td>
						<td>发布时间：</td>
						<td><input type="text" id="search_announcement_time"
							name="search_announcement_time"
							value="<%=request.getAttribute("search_announcement_time") != null ? request
					.getAttribute("search_announcement_time") : ""%>" /></td>
						<td><input type="submit" class="clickbutton" value="搜索"></td>
					</tr>
				</table>
			</form>
			<form action="UpdateAnnouncementServlet">
				<table class="listtable">
					...
					<caption>
						<a class="clickbutton" href="./SystemAnnouncementServlet?type=add">新增公告</a>
					</caption>
					<tr class="listheader">
						<th>编号</th>
						<th>标题</th>
					...
					<tr>
						<%
							String show_announcement_state = null;
								switch (announcementBean.getAnnouncement_state()) {
								case -1:
									show_announcement_state = "已下架";
									break;
								case 0:
									show_announcement_state = "待发布";
									break;
								case 1:
									show_announcement_state = "已发布";
									break;
								}
						%>
						<td><%=announcementBean.getAnnouncement_id()%></td>
						<td><%=announcementBean.getAnnouncement_name()%></td>
						<td><%=show_announcement_state%></td>
						<td><%=announcementBean.getAnnouncement_time() != null ? announcementBean
						.getAnnouncement_time() : "尚未发布"%></td>
						<td><a class="clickbutton"
							href="./SystemAnnouncementServlet?type=update&announcement_id=<%=announcementBean.getAnnouncement_id()%>">编辑</a>
							<a class="clickbutton"
							href="./ReleaseAnnouncementServlet?announcement_id=<%=announcementBean.getAnnouncement_id()%>">发布</a>
							<a class="clickbutton"
							href="./ShelveAnnouncementServlet?announcement_id=<%=announcementBean.getAnnouncement_id()%>">下架</a>
							<a class="clickbutton"
							href="./DeleteAnnouncementServlet?announcement_id=<%=announcementBean.getAnnouncement_id()%>">删除</a></td>
					</tr>
					<%
						}
					%>
				...
				<div class="pager-header">
					<div class="header-nav">
						<a
							href="./SystemAnnouncementServlet?pageno=1&search_announcement_name=<%=request.getAttribute("search_announcement_name")%>&search_announcement_time=<%=request.getAttribute("search_announcement_time")%>">首页</a>
						<a
							href="./SystemAnnouncementServlet?pageno=<%=pageBean.getPageno() - 1%>&search_announcement_name=<%=request.getAttribute("search_announcement_name")%>&search_announcement_time=<%=request.getAttribute("search_announcement_time")%> ">上一页</a>
						<a
							href="./SystemAnnouncementServlet?pageno=<%=pageBean.getPageno() + 1%>&search_announcement_name=<%=request.getAttribute("search_announcement_name")%>&search_announcement_time=<%=request.getAttribute("search_announcement_time")%>">下一页</a>
						<a
							href="./SystemAnnouncementServlet?pageno=<%=pageBean.getMaxpage()%>&search_announcement_name=<%=request.getAttribute("search_announcement_name")%>&search_announcement_time=<%=request.getAttribute("search_announcement_time")%>">尾页</a>
					</div>
				</div>
				<%
					} else {
				...
				<div align="center"
					...
					<table>
						...
						<tr>
							<td><input type="text" name="announcement_id"
								id="announcement_id"
								value="<%=request.getAttribute("announcement_id") != null ? request
					.getAttribute("announcement_id") : ""%>"
								style="display: none" /></td>
						</tr>
						<tr>
							<td>标题：<input type="text" name="announcement_name"
								id="announcement_name"
								value="<%=request.getAttribute("announcement_name") != null ? request
					.getAttribute("announcement_name") : ""%>" /></td>
						</tr>
						<tr>
							<td><textarea id="announcement_content"
									name="announcement_content" maxlength="800" rows="10"
									cols="180"><%=request.getAttribute("announcement_content") != null ? request
					.getAttribute("announcement_content") : ""%></textarea></td>
						</tr>
						<tr>
