	...
	<div class="page-body">
		...
		<div class="page-content">
			...
			<table class="listtable">
				<caption>
					当前位置->增加项目：<%=request.getAttribute("message") != null ? request
				...
				<tr>
					...
					<td><a class="clickbutton"
						href="./FetchAllCostServlet?id=<%=failTypeBean.getFail_type_id()%>&maintenance_id=<%=request.getAttribute("maintenance_id")%>">选择</a>
					</td>
				</tr>
				<%
					}
			...
			<div class="pager-header">
				<div class="header-nav">
					<a
						href="./FetchAllCostServlet?pageno=1&maintenance_id=<%=request.getAttribute("maintenance_id")%>">首页</a>
					<a
						href="./FetchAllCostServlet?pageno=<%=pageBean.getPageno() - 1%>&maintenance_id=<%=request.getAttribute("maintenance_id")%>">上一页</a>
					<a
						href="./FetchAllCostServlet?pageno=<%=pageBean.getPageno() + 1%>&maintenance_id=<%=request.getAttribute("maintenance_id")%>">下一页</a>
					<a
						href="./FetchAllCostServlet?pageno=<%=pageBean.getMaxpage()%>&maintenance_id=<%=request.getAttribute("maintenance_id")%>">尾页</a>
				</div>
			</div>
			<%
				} else {
			...
			<form action="./AddCostServlet">
				<fieldset>
					<legend>新增项目信息</legend>
					<table class="formtable">
						<tr>
							<td><input id="cost_maintenance_id" type="text"
								name="cost_maintenance_id" style="display: none"
								value="<%=request.getAttribute("maintenance_id")%>" /></td>
						</tr>
						<tr>
							<td>项目名称:</td>
							<td><input id="cost_project" type="text" name="cost_project"
								readonly="readonly" style="background-color: lightgray"
								value="<%=request.getAttribute("cost_project") != null ? request
					.getAttribute("cost_project") : ""%>" /></td>
						</tr>
						<tr>
							<td>服务费:</td>
							<td><input id="cost_service" type="text" name="cost_service"
								readonly="readonly" style="background-color: lightgray"
								value="<%=request.getAttribute("cost_service") != null ? request
					.getAttribute("cost_service") : ""%>"></td>
						</tr>
						<tr>
							<td>材料费：</td>
							<td><input id="cost_material" type="text"
								name="cost_material"></td>
						</tr>
						<tr>
