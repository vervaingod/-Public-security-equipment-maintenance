	...
	<div class="page-body">
		...
		<div class="page-content">
			<div class="content-nav">
				当前位置>基本信息 &nbsp;&nbsp;&nbsp;<%=request.getAttribute("message") != null ? request
				...
				<fieldset>
					...
					<table class="formtable">
						<%
							...
							String department = peopleDao
									.findDepartmentById(user_department_id);
						%>
						<tr style="display: block">
							<td>系统单位名称:</td>
							<td><input id="user_department" name="user_department"
								type="text" maxlength="20" readonly="readonly"
								value="<%=department%>" /></td>
						</tr>

						<%
							String show_user_username = null;
							if (user_role != 0) {
								show_user_username = "block";
							} else {
								show_user_username = "none";
							}
						%>
						<tr style="display: <%=show_user_username%>">
							<td>账号:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
							<td><input id="user_username" name="user_username"
								type="text" maxlength="20" readonly="readonly"
								value="<%=peopleBean.getUser_username() != null ? peopleBean
					.getUser_username() : " "%>" /></td>
						</tr>
						<%
							String show_user_name = null;
							if (user_role != 0) {
								show_user_name = "block";
							} else {
								show_user_name = "none";
							}
						%>
						<tr style="display: <%=show_user_username%>">
							...
							<td><input id="user_name" name="user_name" type="text"
								maxlength="20"
								value="<%=peopleBean.getUser_name() != null ? peopleBean
					.getUser_name() : " "%>" /></td>
						</tr>
						<%
							String show_user_phone = null;
							if (user_role != 0) {
								show_user_phone = "block";
							} else {
								show_user_phone = "none";
							}
						%>
						<tr style="display: <%=show_user_phone%>">
							<td>联系电话:</td>
							<td><input id="user_phone" name="user_phone" type="text"
								maxlength="20"
								value="<%=peopleBean.getUser_phone() != null ? peopleBean
					.getUser_phone() : " "%>" /></td>
						</tr>

						<%
							String show_permission = null;
							if (user_role != 0) {
								show_permission = "block";
							} else {
								show_permission = "none";
							}
						%>
						<tr style="display: <%=show_permission%>">
							<td>权限分配:</td>
							<td><input id="user_permission" name="user_permission"
								type="text" maxlength="20" readonly="readonly" /></td>
						</tr>

						<%
							String show_copyright = null;
							if (user_role != 0) {
								show_copyright = "none";
							} else {
								show_copyright = "block";
							}
						%>
						<tr style="display: <%=show_copyright%>">
							<td>版权所有:</td>
							<td><input id="user_copyright" type="text"
								name="user_copyright" maxlength="20"
								value="<%=peopleBean.getUser_copyright() != null ? peopleBean
					.getUser_copyright() : " "%>" /></td>
						</tr>

						<%
							String show_technical_support = null;
							if (user_role != 0) {
								show_technical_support = "none";
							} else {
								show_technical_support = "block";
							}
						%>
						<tr style="display: <%=show_technical_support%>">
							<td>技术支持:</td>
							<td><input id="user_technical_support" type="text"
								name="user_technical_support" maxlength="20"
								value="<%=peopleBean.getUser_technical_support() != null ? peopleBean
					.getUser_technical_support() : ""%>" /></td>
						</tr>
						<tr>
							<td colspan="2" class="command"><input type="submit"
								value="保存" class="clickbutton" /> <input type="reset"
