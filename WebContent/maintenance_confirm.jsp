...
<body>
	...
	<div class="page-body">
		...
		<div class="page-content">
			...
			<form action="MaintenanceConfirmServlet">
				<table class="listtable">
					...
					<tr>
						...
						<%
							...
							departmentBeanlist = departmentDao.fetchAllUnit();
						...
						%>
						<td><select name="maintenance_unit" id="maintenance_unit"><option
									value="">报修公司</option>
								<%
					...
					<tr>
						...
						<td><input id="maintenance_time" type="text"
							value="<%=request.getAttribute("maintenance_time")%>"
				...
				show_maintenance_confirm = "";
									...
									<%=show_maintenance_confirm%>>待确认</option>
								<option value="1"
			...
			<table class="listtable">
				...
				<tr>
					...
					<td><a class="clickbutton"
						href="./FetchMaintenanceConfirmByIdServlet?maintenance_id=<%=maintenanceBean.getMaintenance_id()%>">查看</a>
						<a class="clickbutton"
						href="./ConfirmMaintenanceByMaintenanceID?maintenance_id=<%=maintenanceBean.getMaintenance_id()%>">确认</a></td>
				</tr>
				<%
					}
					} catch (Exception e) {
			...
			<div class="pager-header">
				<div class="header-nav">
					<a href="./MaintenanceConfirmServlet?pageno=1">首页</a> <a
						href="./MaintenanceConfirmServlet?pageno=<%=pageBean.getPageno() - 1%>&maintenance_id=<%=request.getAttribute("maintenance_id")%>&maintenance_unit=<%=request.getAttribute("maintenance_unit")%>&maintenance_fail_type=<%=request.getAttribute("maintenance_fail_type")%>&maintenance_time=<%=request.getAttribute("maintenance_time")%>&maintenance_confirm=<%=request.getAttribute("maintenance_confirm")%>">上一页</a>
					<a
						href="./MaintenanceConfirmServlet?pageno=<%=pageBean.getPageno() + 1%>&maintenance_id=<%=request.getAttribute("maintenance_id")%>&maintenance_unit=<%=request.getAttribute("maintenance_unit")%>&maintenance_fail_type=<%=request.getAttribute("maintenance_fail_type")%>&maintenance_time=<%=request.getAttribute("maintenance_time")%>&maintenance_confirm=<%=request.getAttribute("maintenance_confirm")%>">下一页</a>
					<a
						href="./MaintenanceConfirmServlet?pageno=<%=pageBean.getMaxpage()%>&maintenance_id=<%=request.getAttribute("maintenance_id")%>&maintenance_unit=<%=request.getAttribute("maintenance_unit")%>&maintenance_fail_type=<%=request.getAttribute("maintenance_fail_type")%>&maintenance_time=<%=request.getAttribute("maintenance_time")%>&maintenance_confirm=<%=request.getAttribute("maintenance_confirm")%>">尾页</a>
				</div>
			</div>
			<%
				}
