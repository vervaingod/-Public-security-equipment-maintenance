	...
	<div class="page-body">
		...
		<div class="page-content">
			...
			<table class="listtable">
				<caption>
					维修单位：<%=request.getAttribute("message") != null ? request
					.getAttribute("message") : ""%>
				</caption>
				<caption>
					<a class="clickbutton" href="./MaintainingUnitServlet?type=1">新增单位</a>
				</caption>
				<tr class="listheader">
					<th>账号</th>
					<th>单位名称</th>
				...
				<tr>
					...
					<td><a class="clickbutton"
						href="./MaintainingUnitServlet?id=<%=departmentBean.getDepartment_id()%>">编辑</a>
						<a class="clickbutton"
						href="./InitializePasswordServlet?id=<%=departmentBean.getDepartment_id()%>">初始化密码</a></td>
				</tr>
				<%
					}
				%>
			...
			<div class="pager-header">
				<div class="header-nav">
					<a href="./MaintainingUnitServlet?pageno=1">首页</a> <a
						href="./MaintainingUnitServlet?pageno=<%=pageBean.getPageno() - 1%>">上一页</a>
					<a
						href="./MaintainingUnitServlet?pageno=<%=pageBean.getPageno() + 1%>">下一页</a>
					<a
						href="./MaintainingUnitServlet?pageno=<%=pageBean.getMaxpage()%>">尾页</a>
				</div>
			</div>
			<%
				} else {
			...
			<form action="./UpdateMaintenanceServlet">
				<fieldset style="display: <%=display%>">
					<legend><%=request.getAttribute("message_title") != null ? request
					.getAttribute("message_title") : "单位信息"%></legend>
					<table class="formtable">
						<tr>
							<td>账号:</td>
							<td><input id="department_username"
								value="<%=request.getAttribute("department_username")%>"
								name="department_username" type="text" placeholder="登录账号"
					...
					.getAttribute("department_tel") : ""%>"
						...
						<tr>
							<td colspan="2" class="command"><input type="submit"
								value="<%=request.getAttribute("message_button") != null ? request
					.getAttribute("message_button") : "修改"%>"
								class="clickbutton" /> <input type="reset" value="取消"
