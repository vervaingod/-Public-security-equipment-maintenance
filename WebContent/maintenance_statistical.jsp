	...
	<div class="page-body">
		...
		<div class="page-content">
			...
			<table class="listtable">
				...
				<tr>
					<td>统计信息：</td>
					<td>总数：<%=request.getAttribute("count_sum")%></td>
					<td>已派工：<%=request.getAttribute("count_sent_worker")%></td>
					<td>已作废：<%=request.getAttribute("count_cancellation")%></td>
					<td>待受理：<%=request.getAttribute("countWaitAccept")%></td>
					<td>已受理：<%=request.getAttribute("countAccepted")%></td>
					<td>待确认：<%=request.getAttribute("countWaitConfirm")%></td>
					<td>已确认：<%=request.getAttribute("countConfirmed")%></td>
				</tr>
			</table>
			...
			<form action="MaintenanceStatisticalServlet">
				<table class="listtable">
					<tr>
						...
						<td><select name="maintenance_department"
							id="maintenance_department"><option value="">报修单位</option>
								<%
				...
				show_maintenance_confirm = "";
						...
						<%
							List<DepartmentBean> departmentBeanlist1 = new ArrayList<DepartmentBean>();
							departmentBeanlist1 = departmentDao.fetchAllUnit();
							String message_maintenance_unit = null;
							String show_maintenance_unit = null;
							if (request.getAttribute("maintenance_unit") != null) {
								message_maintenance_unit = request.getAttribute(
										"maintenance_unit").toString();
							}
						%>
						<td><select id="maintenance_unit" name="maintenance_unit"><option
									value="">维修公司搜索</option>
								<%
									for (DepartmentBean departmentBean : departmentBeanlist1) {
								%>
								<option value="<%=departmentBean.getDepartment_name()%>"
									<%if (departmentBean.getDepartment_name().equals(
						message_maintenance_unit)) {
					show_maintenance_unit = "selected='selected'";

				} else {
					show_maintenance_unit = "";
				}%>
									<%=show_maintenance_unit%>><%=departmentBean.getDepartment_name()%></option>
								<%
									}
								%>
						</select></td>
			...
			<table class="listtable">
				...
				<tr>
					...
					<td><a class="clickbutton"
						href="./FetchMaintenanceStatisticalByIdServlet?maintenance_id=<%=maintenanceBean.getMaintenance_id()%>">查看</a>
					</td>
				</tr>
				<%
					}
			...
			<div class="pager-header">
				<div class="header-nav">
					<a
						href="./MaintenanceStatisticalServlet?pageno=1&maintenance_id=<%=request.getAttribute("maintenance_id")%>&maintenance_department=<%=request.getAttribute("maintenance_department")%>&maintenance_fail_type=<%=request.getAttribute("maintenance_fail_type")%>&maintenance_dispatch_state=<%=request.getAttribute("maintenance_dispatch_state")%>&maintenance_time=<%=request.getAttribute("maintenance_time")%>&maintenance_accept_state=<%=request.getAttribute("maintenance_accept_state")%>&maintenance_confirm=<%=request.getAttribute("maintenance_confirm")%>&maintenance_unit=<%=request.getAttribute("maintenance_unit")%>">首页</a>
					<a
						href="./MaintenanceStatisticalServlet?pageno=<%=pageBean.getPageno() - 1%>&maintenance_id=<%=request.getAttribute("maintenance_id")%>&maintenance_department=<%=request.getAttribute("maintenance_department")%>&maintenance_fail_type=<%=request.getAttribute("maintenance_fail_type")%>&maintenance_dispatch_state=<%=request.getAttribute("maintenance_dispatch_state")%>&maintenance_time=<%=request.getAttribute("maintenance_time")%>&maintenance_accept_state=<%=request.getAttribute("maintenance_accept_state")%>&maintenance_confirm=<%=request.getAttribute("maintenance_confirm")%>&maintenance_unit=<%=request.getAttribute("maintenance_unit")%>">上一页</a>
					<a
						href="./MaintenanceStatisticalServlet?pageno=<%=pageBean.getPageno() + 1%>&maintenance_id=<%=request.getAttribute("maintenance_id")%>&maintenance_department=<%=request.getAttribute("maintenance_department")%>&maintenance_fail_type=<%=request.getAttribute("maintenance_fail_type")%>&maintenance_dispatch_state=<%=request.getAttribute("maintenance_dispatch_state")%>&maintenance_time=<%=request.getAttribute("maintenance_time")%>&maintenance_accept_state=<%=request.getAttribute("maintenance_accept_state")%>&maintenance_confirm=<%=request.getAttribute("maintenance_confirm")%>&maintenance_unit=<%=request.getAttribute("maintenance_unit")%>">下一页</a>
					<a
						href="./MaintenanceStatisticalServlet?pageno=<%=pageBean.getMaxpage()%>&maintenance_id=<%=request.getAttribute("maintenance_id")%>&maintenance_department=<%=request.getAttribute("maintenance_department")%>&maintenance_fail_type=<%=request.getAttribute("maintenance_fail_type")%>&maintenance_dispatch_state=<%=request.getAttribute("maintenance_dispatch_state")%>&maintenance_time=<%=request.getAttribute("maintenance_time")%>&maintenance_accept_state=<%=request.getAttribute("maintenance_accept_state")%>&maintenance_confirm=<%=request.getAttribute("maintenance_confirm")%>&maintenance_unit=<%=request.getAttribute("maintenance_unit")%>">尾页</a>
				</div>
			</div>
			<%
				} else {
